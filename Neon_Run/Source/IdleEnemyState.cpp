...
void IdleEnemyState::update(Enemy& enemy) {
	if (!enemy.m_isDead)
	{
		if (enemy.m_animated_sprite.getPosition().x < 1800.0f && !enemy.m_isPlayerDead)
		{
			if (m_clock.getElapsedTime().asSeconds() > 1.0f) {
				if (enemy.m_action == Enemy::Action::SWORD)
				{
				...
				else
				{
					//DEBUG_MSG("IdleEnemyState -> ThrowAttackEnemyState");
					EnemyState* temp = enemy.getEnemyState();
					EnemyState* state = new AttackEnemyState();
					enemy.getEnemyState()->exit(enemy);
					enemy.setEnemyState(state);
					enemy.getEnemyState()->enter(enemy);
					delete temp; // Delete previous
...
void IdleEnemyState::enter(Enemy& enemy)
{
	...
	enemy.getAnimatedSprite().addFrame(sf::IntRect(0, 0, 400, 970));
	enemy.getAnimatedSprite().addFrame(sf::IntRect(400, 0, 400, 970));
	enemy.getAnimatedSprite().addFrame(sf::IntRect(400 * 2, 0, 400, 970));
	enemy.getAnimatedSprite().addFrame(sf::IntRect(400 * 3, 0, 400, 970));
	enemy.getAnimatedSprite().addFrame(sf::IntRect(400 * 4, 0, 400, 970));
	enemy.getAnimatedSprite().addFrame(sf::IntRect(400 * 5, 0, 400, 970));
	enemy.getAnimatedSprite().addFrame(sf::IntRect(400 * 6, 0, 400, 970));
