...
sf::Sprite livesBg;
sf::Texture livesBgTexture;

sf::Texture lives_texture;
sf::Texture noLife_texture;

std::vector<sf::Sprite> lives;
std::vector<sf::Sprite> noLives;

sf::Sprite daggerBg;
sf::Texture daggerBgTexture;
...
std::vector<candle::RadialLight> lights;

void Game::init(sf::RenderWindow& window, Audio& audio)
{
	srand(time(NULL));
...
#pragma region TextureLoading
	...
	if (!bg1_texture.loadFromFile("assets\\bg\\bg1.png")) {
		DEBUG_MSG("Failed to load file");
	}

	if (!bg2_texture.loadFromFile("assets\\bg\\bg2.png")) {
		DEBUG_MSG("Failed to load file");
	}

	if (!bg3_texture.loadFromFile("assets\\bg\\bg3.png")) {
		DEBUG_MSG("Failed to load file");
	}
...
#pragma endregion
	...
	for (size_t i = 0; i < tileArray.size(); i++)
	{
		...
		tileArray[i]->getEnemy().getAnimatedSprite().setScale(.2f, .2f);
		tileArray[i]->getMedkit().getSprite().setScale(.4f, .4f);
	...
	if (!livesBgTexture.loadFromFile("assets/hud/healthBox.png"))
	{
		// Error
	}

	livesBg.setTexture(livesBgTexture);
	livesBg.setScale(.15f, .15f);
	livesBg.setPosition(20.0f, 20.0f);

	if (!lives_texture.loadFromFile("assets/hud/heart.png"))
	{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		life.setScale(.15f, .15f);
		lives.push_back(life);
	}

	lives[0].setPosition(140.0f, 70.0f);
	lives[1].setPosition(180.0f, 70.0f);
	lives[2].setPosition(220.0f, 70.0f);

	if (!noLife_texture.loadFromFile("assets/hud/dimHeart.png"))
	{
		DEBUG_MSG("Failed to load lives texture");
	}
	...
	{
		sf::Sprite noLife;
		noLife.setTexture(noLife_texture);
		noLife.setScale(.15f, .15f);
		noLives.push_back(noLife);
	}

	noLives[0].setPosition(140.0f, 70.0f);
	noLives[1].setPosition(180.0f, 70.0f);
	noLives[2].setPosition(220.0f, 70.0f);

	if (!daggerBgTexture.loadFromFile("assets/hud/knifeBox.png"))
	{
		// Error
	}

	daggerBg.setTexture(daggerBgTexture);
	daggerBg.setScale(.15f, .15f);
	daggerBg.setPosition(window.getSize().x - daggerBg.getGlobalBounds().width - 30.0f, 150.0f);
	...
	daggersHUD.setPosition(daggerBg.getPosition().x + daggerBg.getGlobalBounds().width / 2.0f - daggersHUD.getGlobalBounds().width / 2.0f, daggerBg.getPosition().y + daggerBg.getGlobalBounds().height / 2.0f);

	daggersHUDText.setFont(font);
	daggersHUDText.setCharacterSize(30.0f);
	daggersHUDText.setString("0");
	daggersHUDText.setPosition(daggersHUD.getPosition().x + daggersHUD.getGlobalBounds().width - 10.0f, daggerBg.getPosition().y + daggerBg.getGlobalBounds().height / 2.0f);
	daggersHUDText.setFillColor(sf::Color::Black);

	audio.playBgMusic();

	if (!bloodTexture.loadFromFile("assets/images/bloodsplat.png"))
	{
		// Error
	}
	...
	for (size_t i = 0; i < tileArray.size(); i++)
		lights.push_back(tileArray[i]->getEnemy().getLight());
}

void Game::update(sf::RenderWindow& window, GAME_STATE& gameState, Audio& audio)
{
	...
	for (size_t i = 0; i < tileArray.size(); i++)
	{
		lights[i].setPosition(tileArray[i]->getEnemy().getAnimatedSprite().getPosition().x + tileArray[i]->getEnemy().getAnimatedSprite().getGlobalBounds().width / 2.0f,
							  tileArray[i]->getEnemy().getAnimatedSprite().getPosition().y + tileArray[i]->getEnemy().getAnimatedSprite().getGlobalBounds().height / 2.0f);

		if(tileArray[i]->getEnemy().getAnimatedSprite().getScale().x == .0f)
			lights[i].setScale(.0f, .0f);
		else
			lights[i].setScale(1.0f, 1.0f);
		...
		if (rectangle_to_rectangle(rPlayer, tileArray[i]->getEnemy().m_rectangle))
		{
			if (player.m_isAttacking && !player.m_isDead)
			{
				...
				player.addReward(window, "SWORD KILL\n   +500", rewards);
				score.bonus(scoreBonus + 500);
			...
			if (tileArray[i]->getEnemy().m_isAttacking && !player.m_isDead && !minusOneLife)
			{
				lives.pop_back();
				player.m_health--;
				minusOneLife = true;
		...
		if (player.getAnimatedSprite().getGlobalBounds().intersects(tileArray[i]->getMedkit().getSprite().getGlobalBounds()))
		{
			if (lives.size() < 3 && lives.size() > 0)
			{
				...
				life.setTexture(lives_texture);
				life.setScale(.15f, .15f);
				life.setPosition(lives[lives.size() - 1].getPosition().x + 40.0f, 70.0f);
				lives.push_back(life);
		...
		if (isEnemyHit)
		{
			enemyBlood.setPosition(tileArray[hitEnemy]->getEnemy().getAnimatedSprite().getPosition().x, tileArray[hitEnemy]->getEnemy().getAnimatedSprite().getPosition().y);
		...
		if (player.m_daggers_rectangles.size() > 0)
		{
			...
			if (rectangle_to_rectangle(player.m_daggers_rectangles[0], tileArray[i]->getEnemy().m_headRectangle))
			{
				tileArray[i]->getEnemy().m_headRectangle.setWidth(.0f);
				tileArray[i]->getEnemy().m_headRectangle.setHeight(.0f);

				player.addReward(window, "HEADSHOT\n   +2000", rewards);
				score.bonus(2000);
			}
		}
		...
		if (killStreakCounter > 0 && killStreakCounter % killStreakNb == 0)
		{
			if (!killStreaked)
			{
				...
				score.bonus(1000 * (killStreakCounter / killStreakNb));
				killStreaked = true;
			}
		}
		...
		if (tileArray[i]->getTiles()[tileArray[i]->getTiles().size() - 1].getPosition().x < -tileArray[i]->getTiles()[tileArray[i]->getTiles().size() - 1].getGlobalBounds().width)
		{
			...
			tileArray[i]->getEnemy().getAnimatedSprite().setPosition(window.getSize().x + newEnemyPos.x, newEnemyPos.y - 970.0f * tileArray[i]->getEnemy().getAnimatedSprite().getScale().y + 5.0f);
			tileArray[i]->getMedkit().getSprite().setPosition(window.getSize().x + 850.0f, tileArray[i]->getMedkit().getSprite().getPosition().y);
			tileArray[i]->getExtraKunais().getSprite().setPosition(window.getSize().x + 620.0f, tileArray[i]->getExtraKunais().getSprite().getPosition().y);
	...
	if (isPlayerHit)
	{
		playerBlood.setPosition(player.getAnimatedSprite().getPosition().x, player.getAnimatedSprite().getPosition().y - 5.0f);
	...
	/*daggersHUDText.setPosition(window.getSize().x - daggersHUDText.getGlobalBounds().width - 30.0f, 150.0f);

	daggersHUD.setPosition(daggersHUDText.getPosition().x - daggersHUD.getGlobalBounds().width - 20.0f, 175.0f);*/

	if (bg1_sprite.getPosition().x <= -bg1_sprite.getGlobalBounds().width && allowPositionChangeBg1)
	{
		bg1_sprite.setPosition(sf::Vector2f(bg3_sprite.getPosition().x + bg3_sprite.getGlobalBounds().width - 3.0f, .0f));
		allowPositionChangeBg2 = true;
